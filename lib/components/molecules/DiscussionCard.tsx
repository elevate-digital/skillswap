...
export function DiscussionCard({ item }: { item: SkillType }) {
  ...
  const { user } = useAuth(); // Haal de ingelogde gebruiker op uit de auth context
  const { toggleSkillStatus } = useSkills(); // Haal de functie voor status bepaling van een skill op

  // Check of de ingelogde gebruiker de eigenaar is van de skill
  const isOwner = user?.email === item.user.email;

  return (
    <article className={`flex flex-col gap-[1.3em] items-start bg-[var(--third-bg-color)] p-[25px] md:p-[40px] rounded-[var(--border-radius-md)] border-l-10 ${item.type === "OFFER" ? "border-l-[var(--secondary-bg-color)]" : "border-l-[#B0A4D0]"}`}>
      
      {/*  */}
      <section className="flex gap-3 flex-col-reverse md:flex-row items-baseline md:items-center justify-between w-full">
        <h2 className="flex items-center gap-5 !text-[18px] md:!text-[24px]">
          <IconBg icon={item.type === "OFFER" ? StarIcon : QuestionIcon} bg={item.type === "OFFER" ? "#EBF2E1" : "#F3EFFE"} />
          {item.title}
        </h2>
      ...
      {/* Gebruiker + datum + comments */}
      <section className="flex items-center justify-between w-full mt-2">
        <div className="flex items-center gap-3">
          <ProfilePicture name={item.user.name} />
          <div className="flex flex-col">
            <span className="text-[16px] font-medium text-[var(--primary-text-color)]">{item.user.name}</span>
            <span className="flex flex-row items-center gap-1 text-[14px] text-[var(--primary-text-color)]"><CalendarIcon />{new Date(item.created_at).toLocaleDateString("nl-NL")}</span>
          </div>
        </div>

        {/* Wordt later dynamisch gemaakt */}
        <a className="flex items-center gap-1 text-[var(--primary-text-color)] bg-[var(--primary-bg-color)] rounded-full px-[12px]" href={`/comment/${item.id}`}><ChatCircleIcon className="w-4 h-4" />{item._count?.comments ?? 0}</a>
      </section>
