  ...
  return (
    <article className={`flex flex-col gap-[1.3em] items-start bg-[var(--third-bg-color)] p-[25px] md:p-[40px] rounded-[var(--border-radius-md)] border-l-10 ${item.type === "OFFER" ? "border-l-[var(--secondary-bg-color)]" : "border-l-[#B0A4D0]"}`}>
      ...
      <section className="flex items-center justify-between w-full mt-2">
        ...
        <a className="flex items-center gap-1 text-[var(--primary-text-color)] bg-[var(--primary-bg-color)] rounded-full px-[12px]" href={`/comment/${item.id}`}>
          <ChatCircleIcon className="w-4 h-4" />
          {item._count?.comments ?? 0}
          <span className="sr-only">
            Bekijk {item._count?.comments ?? 0} reacties
          </span>
        </a>
      </section>
